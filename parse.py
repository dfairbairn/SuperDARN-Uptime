...
def process_rawacfs_day(year, month, day, station_code=None, conn=sqlite3.connect("superdarntimes.sqlite")):
    """
    ...
    :param year: [int] year of date to fetch and process
    :param month: [int] month of date to fetch and process
    :param day: [int] day of date to fetch and process 
    [:param station_code:] 3-letter [string] for the station e.g. 'sas' for saskatoon
    [:param conn:]: sqlite connection to a database to put processed entries into

    """
    import subprocess
    import calendar 

    # If a station code is given to function, then just grab that station's stuff
    all_stids = True if station_code is None else False

    # Run the globus connect process
    rut.globus_connect()

    # Fetch the files using Kevin's Globus sync script
    if all_stids:
        script_query = [rut.SYNC_SCRIPT_LOC,'-y', str(year), '-m', str(month), '-p', 
            str(year)+"{:02d}".format(month)+"{:02d}".format(day)+"*", rut.ENDPOINT]
    else:
        # The case that we're looking at just one particular station
        script_query = [rut.SYNC_SCRIPT_LOC,'-y', str(year), '-m', str(month), '-p', 
            str(year)+"{:02d}".format(month)+"{:02d}".format(day)+"*"+station_code, 
            rut.ENDPOINT]
    rut.globus_query(script_query)

    # Parse the rawacf files, save their metadata in our DB
    ...
    # Clear the rawacf files that were fetched in this cycle
    try:
        rut.clear_endpoint()
        logging.info("\t\tDone with clearing {0}-{1}-{2} rawacf data".format(
                 str(year), "{:02d}".format(month), "{:02d}".format(day)))
    except subprocess.CalledProcessError:
        logging.error("\t\tError removing files.", exc_info=True)
...
def process_rawacfs_month(year, month, conn=sqlite3.connect("superdarntimes.sqlite"),
                         multiprocess=True, days=[], station_code=None):
    """
    Takes month and year as arguments. Optionally takes a subset of the month's
    days (list of ints). Steps through each day, fetching, parsing, and clearing.
    ...
    ** On Maxwell this has taken upwards of 14 hours to run for a given month **
    ...
    # For the given month, do calculations to determine its first and last day
    last_day = calendar.monthrange(year, month)[1]
    if type(days)==list and len(days) > 0: 
        cond1 = all([ type(d)==int for d in days])
        cond2 = all([ d in np.arange(1,last_day+1)])
        if cond1 and cond2:
            # Now has the custom days range been fully validated
            all_days = False 
            days_list = days
        else:
            logging.error("Specified days subset is invalid for that year/month combo")
            all_days = True
    elif days==[] or all_days==True:
...
def process_file(fname, conn=sqlite3.connect("superdarntimes.sqlite")):
    ...
    :param f: [str] file name including path.
    [:param conn:]: [sqlite3 connection] to the desired database

    **Note**
        This function uses the exception handler process similar to parse folder
        despite only one file being processed in order to simplify code re-use. 
    """
    ...
    write_handler.start()
    try:
        # Start a single call of parse_file
        dummy_index = 1
        ...
        # Handle a failed backscatter read 
        # **NOTE** this should be handled already by parse_file
        ...
        # Handle a detected inconsistency in the file
        # **NOTE** this should be handled already by parse_file
...
def parse_rawacf_folder(folder, conn=sqlite3.connect("superdarntimes.sqlite"), 
    ...
    :param folder: [str] indicating the path/name of a folder holding rawacf files
    [:param conn:] [sqlite3 connection] to the database
    [:param multiprocess:] [Boolean] whether or not to use a multiprocessing pool
    """
    from contextlib import closing # Forces processes to close, else memory errors occur
    assert(os.path.isdir(folder))
    conn.text_factory = str # Specifies tool for sqlite to use in converting text
    cur = conn.cursor()
    ...
    if multiprocess==False:
        ...
        for i, fil in enumerate(files):
            ...
            recs.append(r)
    # Saving to the database and counting the number of rawacf files that weren't counted
    num_uncounted = 0
    for rec in recs:
        if rec is not None:
            rec.save_to_db(cur)
    ...
    *NOTE*: Contrary to convention, _all_ exceptions are handled using umbrella
    ...
    try:
        ...
        if r.not_corrupt == False:
            # Call check_fields again to see the problems
            objection_dict = rut.check_fields(dics) 
            err_str = 'Data inconsistency encountered:\n{1}'
            raise rut.InconsistentRawacfError(err_str.format(fname, objection_dict))
        logging.info('\t{0} File  {1}: File processed cleanly.'.format(index, fname))
        ...
        exc_msg_queue.put((fname, e))
        if isinstance(e, rut.BadRawacfError): # Rare - usually linked to single-entry dmaps
    ...
    while True:
        if exc_msg_queue.empty():
            ...
            try:
                ...
                check_inconsistent = isinstance(exc, rut.InconsistentRawacfError)
                check_badrawacf1 = isinstance(exc, backscatter.dmap.DmapDataError)
                check_badrawacf2 = isinstance(exc, rut.BadRawacfError)
                if check_inconsistent:
                    logging.debug("\t\tWrite handler saving a bad_cpid event")
                    write_inconsistent_rawacf(fname, exc)
                elif check_badrawacf1 or check_badrawacf2:
...
def write_inconsistent_rawacf(fname, exc, inconsistents_log=INCONSISTENT_FIELDS_FILE):
    ...
    [:inconsistents_log:]: [str] name of file to log inconsistent-fields errors to
    """
...
def write_bad_rawacf(fname, exc, bad_files_log=BAD_RAWACFS_FILE): 
    ...
    [:param bad_files_log:]: [str] name of file to log bad rawacf errors to 
    """
    # ***ADD TO LIST OF BAD_RAWACFS ***
    with open(bad_files_log, 'a') as f:
        # Backscatter exceptions have a newline that looks bad in logs, so I remove them here
